...
export default function ProfileRemove({options, args}: Props) {
	const [name] = args;
	...
	useEffect(() => {
		...
		if (!config.profiles?.[name!]) {
			...
			setResult({ok: false, message: `Profile "${name}" not found.`});
			return;
		}
		...
		setResult({ok: true, message: `Profile "${name}" removed`});
	}, []);
